...
- Trips list (home): hero area plus cards with cover image placeholder; when there is an in-progress trip, show `CurrentTripCard` and a compact pinned header (`CompactCurrentTrip`) that appears once the hero card scrolls out of view; otherwise show a single `CountdownCard(heroStyle = true)` hero item for the next upcoming trip; list cards show status labels (no per-card countdown chips); empty state CTA; bottom-bar + -> TripForm(new); tap card -> TripDetail.
